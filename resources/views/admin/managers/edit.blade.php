...
<div class="max-w-lg">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">매니저 수정</h1>

    <form action="/admin/managers/{{ $manager->id }}" method="POST" class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="password" name="password"
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <p class="mt-1 text-xs text-gray-400">변경할 경우에만 입력하세요</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">텔레그램 채팅 ID</label>
                <input type="text" name="telegram_chat_id" value="{{ old('telegram_chat_id', $manager->telegram_chat_id) }}"
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ $manager->is_active ? 'checked' : '' }}
                    class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                <label for="is_active" class="text-sm text-gray-700">활성화</label>
            </div>

            <div class="pt-4 border-t border-gray-100">
                <p class="text-sm text-gray-500">담당 리드: <strong class="text-gray-700">{{ $manager->leads_count }}건</strong></p>
                <p class="text-sm text-gray-500">등록일: <strong class="text-gray-700">{{ $manager->created_at?->format('Y-m-d H:i') }}</strong></p>
            </div>
        </div>
